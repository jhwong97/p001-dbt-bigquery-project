...
#### Task 4: Using dbt to Transform Data
The following steps are needed to perform the data transformation using dbt:
- Create the following directories:
    - `include/dbt`
    - `include/dbt/models`
    - `include/dbt/models/sources`
    - `include/dbt/models/transforms`
- Create a `dbt_project.yml` refer to [this](include/dbt/dbt_project.yml) in `include/dbt`.
- Create a `packages.yml` refer to [this](include/dbt/profiles.yml) `include/dbt`.
- Create a `profiles.yml` in `include/dbt` with the following codes:
```yaml
retail:
  target: dev
  outputs:
    dev:
      ...
      project: 'airflow-project-414407'
      dataset: retail
      threads: 1
      timeout_seconds: 300
      location: US
```
Make sure to replace `keyfile`, `location` and  `project` with your own values.
- Go to Bigquery on GCP, then create a new SQL query as below and execute it. The SQL request creates a new table called `country` which store all the necessary information related to it.
```sql
...
```
- Create a `sources.yml` in `include/dbt/models/sources` with the following codes:
```yaml
version: 2

sources:
  - name: retail
    database: airflow-project-414407

    tables:
      - name: raw_invoices
      - name: country
```
Make sure to replace `database` with your own project id.
- Create a `dim_customer.sql` refer to [this](/include/dbt/models/transforms/dim_customer.sql) in `include/dbt/models/transforms`.
- Create a `dim_datetime.sql` refer to [this](/include/dbt/models/transforms/dim_datetime.sql) in `include/dbt/models/transforms`.
- Create a `dim_product.sql` refer to [this](/include/dbt/models/transforms/dim_product.sql) in `include/dbt/models/transforms`.
- Create a `fct_invoices.sql` refer to [this](/include/dbt/models/transforms/fct_invoices.sql) in `include/dbt/models/transforms`.
...
```
- Create a `cosmos_config.py` refer to [this](include/dbt/cosmos_config.py) for integrating the cosmos into the airflow.
